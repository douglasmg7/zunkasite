...
const PAGE_SIZE = 50;
const DIR_IMG_PRODUCTS = path.join(__dirname, '..', '/dist/img/manual/products');
...
router.get('/', function (req, res) {
  ...
  const cursor = mongo.db.collection(dbConfig.collManualProducts).find(search).sort({'desc': 1}).skip(skip).limit(PAGE_SIZE);
  Promise.all([
    cursor.toArray(),
    cursor.count()
  ]).then(([products, count])=>{
...
router.get('/:id', function(req, res) {
  mongo.db.collection(dbConfig.collManualProducts).findOne({code: '0059989'}, (err, r)=>{
    if(err){
...
router.put('/set-commercialize/:_id', function(req, res) {
  ...
  // Update manual products in store products database.
  mongo.db.collection(dbConfig.collManualProducts).findOneAndUpdate({_id: _id},
    {$set: {commercialize: commercialize}},
    {returnOriginal: false})
  // Upsert store products database.
  .then(result=>{

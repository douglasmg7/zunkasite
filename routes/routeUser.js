...
// Edit CNPJ page.
router.get('/access/edit-cnpj', checkPermission, (req, res, next)=>{
    res.render('user/editCnpj', { cnpj: req.user.cnpj, invalid: {} });
});

// Edit CNPJ.
router.post('/access/edit-cnpj', checkPermission, (req, res, next)=>{
    // Validation.
    let invalid = {};
    if (!cnpj.isValid(req.body.cnpj)) {
        invalid.cnpj = "Valor inválido";
    }
    // Invalid fields.
    if (Object.keys(invalid).length) {
        return res.render('user/editCnpj', { cnpj: req.body.cnpj, invalid: invalid });
    }
    User.findById(req.user._id, (err, user)=>{
        if (err) { return next(err) };
        if (!user) { return next(new Error('Not found user to save.')); }
        ...
        user.save(function(err) {
            if (err) { return next(err); } 
            return res.redirect('/user/access');
        });  
    });
});

// Edit contact name page.
router.get('/access/edit-contact-name', checkPermission, (req, res, next)=>{
    res.render('user/editContactName', { contactName: req.user.contactName, invalid: {} });
});

// Edit contact name.
router.post('/access/edit-contact-name', checkPermission, (req, res, next)=>{
    // Validation.
    let invalid = {};
    if (req.body.contactName.length < 3) {
        invalid.contactName = "Valor inválido";
    }
    // Invalid fields.
    if (Object.keys(invalid).length) {
        return res.render('user/editContactName', { contactName: req.body.contactName, invalid: invalid });
    }
    User.findById(req.user._id, (err, user)=>{
        if (err) { return next(err) };
        if (!user) { return next(new Error('Not found user to save.')); }
        user.contactName = req.body.contactName.trim();
        user.save(function(err) {
            if (err) { return next(err); } 
            return res.redirect('/user/access');
        });  
...
// Edit state registration.
router.get('/access/edit-state-registration', checkPermission, (req, res, next)=>{
    res.render('user/editStateRegistration', { stateRegistration: req.user.stateRegistration, invalid: {} });
});

// Edit contact name.
router.post('/access/edit-state-registration', checkPermission, (req, res, next)=>{
    // Validation.
    let invalid = {};
    if (req.body.stateRegistration.length > 100) {
        invalid.stateRegistration = "Valor inválido";
    }
    // Invalid fields.
    if (Object.keys(invalid).length) {
        return res.render('user/editStateRegistration', { stateRegistration: req.body.stateRegistration, invalid: invalid });
    }
    User.findById(req.user._id, (err, user)=>{
        if (err) { return next(err) };
        if (!user) { return next(new Error('Not found user to save.')); }
        user.stateRegistration = req.body.stateRegistration.trim();
        user.save(function(err) {
            if (err) { return next(err); } 
            return res.redirect('/user/access');
        });  

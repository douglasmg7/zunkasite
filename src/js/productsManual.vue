<template lang='pug'>
  div
    menu-products(active='manual' v-on:input='search=arguments[0]' v-on:search='getProducts()')
    ...
    .ui.hidden.divider
    products-manual-detail(:product='selectedProduct' @save='updateProduct')
</template>
<script>
  /* globals accounting */
  'use strict';
  ...
  import productsManualDetail from './productsManualDetail.vue';
  ...
  export default {
    ...
    methods: {
      ...
      getProducts(page=1){
        this.$http.get(`${wsPath.manual}?page=${page}&search=${this.search}`)
          .then((res)=>{
